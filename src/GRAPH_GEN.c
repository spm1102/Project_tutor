...
graph_t *GEN_GRAPH_Create(const cell_t **grid)
{
    ...
    // array to check wheter a node is added edge
    ...
    for (int i = 0; i < COLS; i++)
    {
        for (int j = 0; j < ROWS; j++)
        {
            ...
            if(!IS_wall(i, j, grid)) {
                if (i > 0 && !IS_wall(i - 1, j, grid) && EDGE_Added[i - 1][j] == false)                 // Add edge to the above node
                {
                    int topVertex = grid[i - 1][j].vertex;
                    EDGE_Add(p_graph, currentVertex, topVertex, 1);
                }
                if (i < COLS - 1 && !IS_wall(i + 1, j, grid) && EDGE_Added[i + 1][j] == false)          //Add edge to the under node
                {
                    int bottomVertex = grid[i + 1][j].vertex;
                    EDGE_Add(p_graph, currentVertex, bottomVertex, 1);
                }
                if (j > 0 && !IS_wall(i, j - 1, grid) && EDGE_Added[i][j - 1] == false)                 //Add edge to the left node
                {
                    int leftVertex = grid[i][j - 1].vertex;
                    EDGE_Add(p_graph, currentVertex, leftVertex, 1);
                }
                if (j < ROWS - 1 && !IS_wall(i, j + 1, grid) && EDGE_Added[i][j + 1] == false)          //Add edge to the right node
                {
